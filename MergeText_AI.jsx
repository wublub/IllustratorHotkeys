...
var MERGE_TEXT_AI_QUICK_MODE = (typeof MERGE_TEXT_AI_QUICK != "undefined" && MERGE_TEXT_AI_QUICK === true);
...
if(app.documents.length >= 1) {
	...
	var tfs  = new Array();
	var separator = "[><]";
	if(MERGE_TEXT_AI_QUICK_MODE) separator = "";
	var lineTolerance = 3;
...
function init(){
	collectFields();
	if(tfs.length> 1) {
		if(aiversion >= minimumversion && !MERGE_TEXT_AI_QUICK_MODE) runDialog();
		else run();
...
function runDialog()
{
	...
	omd.number("Y tolerance (pt)","lineTolerance",lineTolerance);
	omd.boxedText(2,"\\r = return, \\t = tab" + "\r" + "Rows with different Y positions become separate lines.");
	...
	if(result != null) {
		...
		//separator = replace(separator, "\\n", "\n");
		lineTolerance = parseFloat(result.lineTolerance);
		if(isNaN(lineTolerance) || lineTolerance < 0) lineTolerance = 3;
...
function assemble(arr){
	var ordered = orderFieldsByRows(arr);
	var targField = ordered.shift().field;
	for (var i=0; i<ordered.length; i++){
		var prefix = ordered[i].newLine ? "\r" : separator;
		ordered[i].field.textRange.contents = prefix + ordered[i].field.textRange.contents;
		ordered[i].field.textRange.move(targField, ElementPlacement.PLACEATEND);
		ordered[i].field.remove();
	}
}

function orderFieldsByRows(arr) {
	var rows = new Array();
	var tol = parseFloat(lineTolerance);
	if(isNaN(tol) || tol < 0) tol = 3;

	for(var i=0; i<arr.length; i++) {
		var field = arr[i];
		var y = parseFloat(field.top);
		var bestRow = null;
		var bestDiff = tol + 1;

		for(var r=0; r<rows.length; r++) {
			var diff = Math.abs(y - rows[r].y);
		...
		if(bestRow == null) {
			bestRow = new Object();
			bestRow.y = y;
			bestRow.fields = new Array();
			rows.push(bestRow);
		} else {
			bestRow.y = ((bestRow.y * bestRow.fields.length) + y) / (bestRow.fields.length + 1);
		}
		bestRow.fields.push(field);
	}

	rows.sort(sortRowsByY);

	var ordered = new Array();
	for(var rowIndex=0; rowIndex<rows.length; rowIndex++) {
		rows[rowIndex].fields.sort(sortFieldsInRow);
		for(var f=0; f<rows[rowIndex].fields.length; f++) {
			var item = new Object();
			item.field = rows[rowIndex].fields[f];
			item.newLine = (rowIndex > 0 && f == 0);
			ordered.push(item);
		}
	}
	return ordered;
...
function sortRowsByY(a,b) {
	if(sortProp == "bottom") return a.y - b.y;
	return b.y - a.y;
}

function sortFieldsInRow(a,b) {
	var aN = parseFloat(a.left);
	var bN = parseFloat(b.left);
	if(sortProp == "right") return bN - aN;
	return aN - bN;
}
