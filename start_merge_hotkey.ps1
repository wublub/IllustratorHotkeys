...
$localScript = Join-Path $PSScriptRoot "MergeText_AI_Quick.jsx"
if ([string]::IsNullOrWhiteSpace($ScriptPath)) {
    $ScriptPath = $localScript
}

if (-not (Test-Path -LiteralPath $ScriptPath)) {
    throw "MergeText_AI_Quick.jsx not found: $ScriptPath"
}
...
public static class AiMergeHotkeyHook
{
    ...
    public static int TargetVkCode = (int)Keys.F2;
...
[AiMergeHotkeyHook]::TargetVkCode = [int]$keyValue
[AiMergeHotkeyHook]::IllustratorExe = (Get-Item -LiteralPath $IllustratorExe).FullName
[AiMergeHotkeyHook]::ScriptPath = (Get-Item -LiteralPath $ScriptPath).FullName

if ($Test) {
    Write-Host "Hotkey helper compiled."
    Write-Host "Key: $Key"
    Write-Host "Illustrator: $([AiMergeHotkeyHook]::IllustratorExe)"
    Write-Host "Script: $([AiMergeHotkeyHook]::ScriptPath)"
    exit 0
}

Write-Host "MergeText_AI hotkey is running."
Write-Host "Key: $Key"
